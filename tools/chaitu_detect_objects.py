...
def vis_detections(im, class_name, dets, coords, thresh=0.5):
	...
	coords['labels'].append( {"_id":len(coords['labels']) + 1, "box": [[int(bbox[1]), int(bbox[0])], [int(bbox[2] - bbox[0]), int(bbox[3] - bbox[1])]], "tag":class_name})
...
def detect_objects(image_path):
    ...
    return demo(net, im_name)

    #plt.show()
    #plt.savefig('/var/services/homes/kchakka/py-faster-rcnn/test_fig')
